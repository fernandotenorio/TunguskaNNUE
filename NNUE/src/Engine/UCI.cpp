...
const std::string UCI::ENGINE_NAME = "Tunguska 2.0";
...
void UCI::uciLoop(){
	...
	while (true){
		...
		if ("uci" == input){
			...
			Perft::runAll("D:\\cpp_projs\\Tunguska\\perft.txt");
		}
		else if ("eval" == input){
			Evaluation::testEval("D:\\cpp_projs\\Tunguska\\unique_sample.fen");
		}
		else if ("bench" == input){
			TestSuite::runFile("D:\\cpp_projs\\Tunguska\\positions\\STS1-STS15_LAN.EPD", 50);
		}
	}
}
